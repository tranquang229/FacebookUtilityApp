...
using FM.Infrastructure.Utils;
using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;

namespace FM.Infrastructure.Seeds
{
    public class SeedData
    {
        ...
        public static async Task SeedDefaultAccount(UserManager<ApplicationUser> userManager, RoleManager<ApplicationRole> roleManager)
        {
        ...
        public static async Task SeedDefaultToken(List<FbToken> listFbTokens, string path, ApplicationDbContext appContext)
        {
            var listToken = ExcelUtil.MakeListFbTokensFromExcelFile("FbTokens.xlsx", path);

            try
            {
                foreach (var fbToken in listToken)
                {
                    var fbTokenInDb = await appContext.FbTokens.FirstOrDefaultAsync(x => x.Uid == fbToken.Uid);
                    if (fbTokenInDb == null)
                    {
                        await appContext.FbTokens.AddAsync(fbToken);
                    }
                    else
                    {
                        fbToken.Id = fbTokenInDb.Id;
                        appContext.Entry(fbTokenInDb).CurrentValues.SetValues(fbToken);
                    }

                    await appContext.SaveChangesAsync();
                }

                //await appContext.BulkUpdateAsync(listFb);
                //await appContext.FbTokens.AddRangeAsync(listFb);
                //await appContext.SaveChangesAsync();
            }
            catch (Exception e)
            {
                throw new ExceptionCustom("SeedDefaultToken", listFbTokens, e.Message, e);
            }
        }
    }
}